...
function Header() {
  ...
  const handleOverlayClick = (e) => {
    if (e.target.classList.contains("watchlist-overlay")) {
      handleOpenWatchList();
    }
  };
  ...
  return (
    <div>
      ...
      {openWatchList && (
        <div
          className="watchlist-overlay fixed inset-0 bg-black bg-opacity-50 z-10 top-[7vh] flex justify-end"
          onClick={handleOverlayClick}
        >
          <div
            className="w-[470px] h-[700px] pb-20 bg_watch flex flex-col"
            ...
            <div className="flex flex-wrap items-center justify-center gap-[25px] overflow-y-scroll no-scrollbar">
              {watchList && watchList.length > 0 ? (
                watchList.map((crypto) => (
                  <div key={crypto.id}>
                    <div className="card w-[200px] h-[310px] flex flex-col mb-[20px] items-center bg-gray-950 rounded-[25px]">
                      <img
                        className="w-[118px] h-[118px] mt-[17px]"
                        src={crypto.image}
                        alt=""
                      />
                      <p className="text-[20px] mt-[35px] font-bold">
                        {crypto.name}
                      </p>
